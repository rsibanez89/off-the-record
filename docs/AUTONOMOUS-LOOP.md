...
- Edit `docs/AUTONOMOUS-LOOP.md` (this file) **without an explicit human
  instruction to do so**. The protocol is owned by the human; the agent
  may update it when the user asks for a clarification or rule change.
- Edit `BENCHMARKS.md` by hand (regenerated by `bench:promote`).

## What counts as an "improvement" from the backlog

The **integration tests are the definitive gate**. `npm run bench`
produces the verdict; promote on improvement, revert on regression.
Nothing ships without going through that gate. Conversely, any change
that passes the gate is fair game to attempt, regardless of how big it
looks.
...
**Refactors are accepted and encouraged** when they follow SOLID
principles and ship with the unit tests that prove the new contracts.
Examples of refactors that fit the loop:

- Extracting a `WhisperEngine` class (2.5) that owns pipeline lifecycle,
  consumed by both the live consumer and the batch worker.
- Splitting `consumer.worker.ts`'s `OutMessage` union (2.6) into
  narrower discriminated unions.
- Moving live-loop constants into `config.ts` (2.7) so per-model
  overrides do not need new code paths.
- Localised data-shape changes inside `HypothesisBuffer` (e.g. a
  per-word agreement counter on `tentative` to require LA-2 agreement
  across DISTINCT audio windows for 5.14).

The gate is the same: integration tests must pass and the bench must
not regress. The protocol's "stick to small, scoped items" guidance is
about *velocity per iteration*, not a ban on refactors. A refactor
counts as one attempt against the 5-attempt cap.

Items that genuinely do NOT fit the loop (the bench cannot see the
change, the work is research-grade, or it depends on something
external):

- 1.x model picker additions (those require download, license review,
  and have UX consequences the bench does not measure).
- 5.4 AlignAtt policy (research-grade; requires cross-attention plumbing
  through transformers.js that the test harness does not currently
  exercise).
- 5.6 KV cache reuse (blocked on upstream transformers.js work).
- 5.7 speculative decoding (research; needs two models in memory).
- 1.7 sherpa-onnx engine (second runtime; architectural and bench fixtures
  are tiny.en only).
- 3.2 pipeline `dispose()` cycle (WebGPU-only behaviour; Node CPU bench
  cannot measure the GPU memory leak). Worth doing, but the bench
  produces a false negative, so keep it off the loop.
